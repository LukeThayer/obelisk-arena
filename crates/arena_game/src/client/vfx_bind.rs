...
//!
//! Ported verbatim from `arena_editor/src/vfx_bind.rs` so the LIVE game bakes stat/charge-driven VFX
//! params identically to the skill-designer preview. `apply_modulated_param` maps a named authoring
//! param onto the first emitter's `bevy_vfx` module stack (`"scale"`→`SetSize`, `"emission"`→
//! `SpawnModule::Rate`, `"color"`→scale `SetColor` RGB); `bake_bindings` resolves each binding's live
//! driver via [`resolve_binding`] and applies it. (The pure normalize/modulate math lives in
//! `arena_skills`; only this bevy_vfx-specific application is duplicated, since arena_skills stays
//! bevy_vfx-free and arena_editor is a separate workspace.)
