...
- `detect_round_end` (`:744`) reads obelisk's `EntityDied` stream **only during `Active`**, credits the *survivor* (the other player), increments scores, transitions. It still drains the stream in other phases so a stale death isn't mis-attributed.
- `run_round_machine` (`:795`) advances by real time and runs `reset_for_new_round` on the Countdown→Active rising edge: heal to max life/mana, clear effects, `interrupt_cast`, teleport both to `SPAWN_MARKERS` by **sorted client-id slot**, zero `LinearVelocity`. The Position reset replicates; the predicted owner rolls back to it.
