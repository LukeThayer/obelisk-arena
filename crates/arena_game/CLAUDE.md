...
| `server/cast_pipeline.rs` | `drain_cast_requests` + `resolve_cast_aim` — resolve a candidate `CastAim` from the skill's authored `Acquisition` (hitscan-entity / ground-point raycast from the eye, else direction); insert a `PendingCast`; obelisk's `validate_casts` does the authoritative range/filter/fallback. |
...
3. `drain_cast_requests` (`server/cast_pipeline.rs`): resolves sender `RemoteId` → caster via `ClientPlayerMap`, skips a caster mid-`ActiveCast`, and resolves a candidate `CastAim` from the skill timeline's `Acquisition` (`resolve_cast_aim`: `HitscanEntity` raycasts the aim ray for a target entity, `GroundPoint` for a ground point, `Aim`/`SelfPoint` cast by direction from the eye `Vec3::Y*ARENA_EYE_HEIGHT`). Inserts a `PendingCast`; obelisk's `validate_casts` → `resolve_acquisition` does the authoritative range/filter/fallback (a direction cast can miss and fizzle).
...
6. Client consume: `consume_replicated_cues` (`skills.rs:199`) is the SINGLE `CueWireMessage` drain — it de-dups the local player's own `OnCast` cue and forwards survivors as `LocalCue`. `predicted_local_cast` (`skills.rs:259`) emits the local on_cast `LocalCue` immediately (never a `Hitbox`, never `CombatRng`). `spawn_cue_cosmetics` (`cosmetics.rs`, C3) resolves each `LocalCue`'s `CueBinding` (keyed by the cue's own `skill_id`) and spawns the bound effect via `EffectLibrary`/`VfxLibrary`, flying a `CosmeticProjectile` for `CueAttach::Follow` bindings. Damage numbers come from the replicated `DamageResolved` in `hud.rs`.
